...
const MAX_TOKENS = 2048; // 2048 es más que suficiente y ahorra tokens de salida
const MAX_TOOL_ROUNDS = 5; // límite de seguridad para evitar loops infinitos de tool calling
const HISTORIAL_MENSAJES = 4; // Bajar a 4 reduce drásticamente el consumo de tokens y es suficiente para el contexto
    ...
    system: [
      {
        type: 'text',
        text: SYSTEM_PROMPT,
        cache_control: { type: 'ephemeral' } // Activar Prompt Caching para el prompt del sistema
      }
    ] as any,
    tools,
    messages,
